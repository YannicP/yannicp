...
Goal of this project was to gain insights into a certain product category by analyzing reviews for respective products on Amazon. The Dataset was first filtered for headphone reviews only. 

#### Preprocessing
Before analysis, the raw reviews have to be cleaned and transformed into a less noisy version. The process taken is shown below:

![Diagram1](images/Amazon/preprocessing_diagram.png)

Afterwards, different analysis approaches were executed. Exploratory Data Analysis and Topic Modeling were combined to identify potential features to target in order to improve customer satisfaction.

To get a first glance on the dataset, word clouds with word frequencies were created:

![Diagram2](images/Amazon/ngrams.png)

#### Analyzing Brand Position
Through word2vec, the brand position was analayzed. This is useful to identify possible targets for marketing. The calculations were generated through projecting the different word embedding vectors generated by the word2vec skip-gram network onto the axis between the two analyzed brands, *bose* and *audiotechnica* (See Bolukbasi et al. 2016 for Reference)

![Diagram3](images/Amazon/brand_positioning.png)

For example, Bose is more associated with instrumental, rock and pop music. This is important information and allows the brand to focus exactly these styles of music, improving the customer experience. It is also visible that customers tend to group budget and premium brands, which seems natural.
...
Through calculating metrics such as the Lift or using tf-idf, the topics can be named.
...
- Gaming seems to be a potential target. Improvements in regards to the headphones gaming performance could drive customer satisfaction further
- Battery Life and comfort are not driving ratings very much based on the regression coefficients. However, they are mentioned more likely in reviews that have only average ratings, therefore the headphones could likely be improved in this regard. 
...
This project is an excerpt from my bachelors thesis. Therefore, no completely comprehensive overview is given.
...
- defect detection has to be fast, as online defect detection should be possible. Therefore no extremely complex calculations can be carried out
...
It is clearly visible that the company's sales are clearly dependent on the season. In November and December, Sales are exceptionally high compared to the rest of the year.
...
The creation of an ARIMA (Auto-Regressive Integrated Moving Average) model requires the selection of specific parameters *p* and *q* that tell it how many autoregressive lags (correlation with previous values) and moving average lags (correlation of the error terms) should be taken into account.

This parameter selection can be achieved by taking a look at the ACF and PACF functions for the time series:
...
- people that paid more, and were passengers in a higher class were more likely to survive.
...
Neural Networks have the disadvantage of overfitting if no countermeasures are taken. Especially when little data is present (as is the case here), they tend to follow the training data too closely, making overfitting a daunting challenge. To combat this, there are various techniques available:
...
1. Input Layer: dropout rate 0.2
...
Setting up the network in tensorflow is fairly straightforward:
 ...
 ```
 The loss funtion used is basic cross entropy, while training is carried out using adaptive moment estimation:
